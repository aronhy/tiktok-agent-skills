...
1. 确认目标类目和目标国家或地区。两者都是**绝对阻断输入**：若类目缺失，先只问“目标类目是什么？它决定研究对象、商品映射和内容比较范围。”并停止；类目已提供而市场缺失时，只问“目标国家或地区是什么？它会改变趋势、价格、政策和竞争格局。”并停止。即使用户要求继续，也不得在两项齐全前调用研究工具、输出通用框架、条件策略、go/no-go、市场主张或正式报告。
...
5. 仅在目标类目和目标市场均已提供后，才完成允许的重试、分页、ID 关联和公开浏览器读取，再检查证据完整性。此后的关键取证缺失会改变进入结论、切入方向或合规判断时，只问一个问题；用户要求继续时可提供带影响范围和 A/B/C 可信度的部分报告。
